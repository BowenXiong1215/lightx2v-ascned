...
class MiniMaxH3T2AVModel(BaseModel):
    ...
    def load_components(
        ...
        *,
        ...
        if load_transformer:
            ...
            # FSDP2 accepts a CPU module and moves only each rank's parameter
            # shard to the device mesh. Avoid materializing the complete H3
            # transformer on every 64 GiB NPU before sharding.
    ...
    def transformer_forward_context(self):
        if self.use_autocast and self.device.type in {"cuda", "npu"} and self.running_dtype in {torch.float16, torch.bfloat16}:
            return torch.autocast(self.device.type, dtype=self.running_dtype)
        return nullcontext()
