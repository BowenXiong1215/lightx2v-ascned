...
class _DmdRuntime(BaseTrainer):
    ...
    def __init__(self, config):
        ...
        self.student_init_lora_path = self.student_config.get("init_lora_path")
        if self.student_init_lora_path is not None:
            if self.student_train_type != "lora":
                raise ValueError("training.student.init_lora_path requires training.student.train_type=lora.")
            if not isinstance(self.student_init_lora_path, str) or not self.student_init_lora_path.strip():
                raise ValueError("training.student.init_lora_path must be a non-empty path string.")
    ...
    def _setup_trainable_model(self, model, role="student"):
        ...
        if role == "student" and self.student_init_lora_path:
            model.load_lora_weights_for_resume(self.student_init_lora_path)
            logger.info("[train] initialized student LoRA from {}", self.student_init_lora_path)
