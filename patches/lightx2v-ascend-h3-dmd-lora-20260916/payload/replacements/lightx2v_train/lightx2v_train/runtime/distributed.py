...
from lightx2v_train.runtime.accelerator import (
    get_accelerator,
...
def init_distributed(config=None):
    ...
    device_type = initialize_accelerator(config, local_rank=local_rank)
    ...
    if _DEVICE_MESH is None:
        ...
        if sp_size > 1:
            ...
            _DEVICE_MESH = init_device_mesh(device_type, (world_size,))
...
def barrier():
    if dist.is_available() and dist.is_initialized():
        if dist.get_backend() in {"nccl", "hccl"} and get_accelerator_type() != "cpu":
            dist.barrier(device_ids=[get_accelerator().current_device()])
        else:
            dist.barrier()
